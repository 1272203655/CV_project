...
class OctaveConv(nn.Layer):
    def __init__(self,in_channels,out_channels,kernel_size,alpha_in=0.5,alpha_out=0.5,
                 stride = 1 ,padding = 0,dilation = 1 , groups=1, bias_attr = False):
        ...
        self.conv_l2l =  None if alpha_in == 0 or alpha_out == 0 else\
            nn.Conv2D(int(alpha_in*in_channels), int(alpha_out*out_channels), kernel_size,1,padding,
                      dilation,math.ceil(alpha_in*groups),bias_attr=bias_attr)
            
        self.conv_l2h = None if alpha_in == 0 or alpha_out == 1 or self.is_dw else \
            nn.Conv2D(int(alpha_in*in_channels), out_channels-int(alpha_out*out_channels), kernel_size,
                      1,padding,dilation,groups,bias_attr=bias_attr)
        
        self.conv_h2l =  None if alpha_in == 1 or alpha_out == 0 or self.is_dw else \
            nn.Conv2D(in_channels - int(alpha_in* in_channels), int(alpha_out*out_channels), kernel_size,
                      1,padding,dilation,groups,bias_attr=bias_attr)
        self.conv_h2h = None if alpha_in == 1 or alpha_out == 1 else \
            nn.Conv2D(in_channels - int(alpha_in* in_channels), out_channels-int(alpha_out*out_channels), 
                      kernel_size,1, padding, dilation, math.ceil(groups - alpha_in * groups), bias_attr=bias_attr)
    def forward(self,x):
...
class Conv_BN(nn.Layer):
    def __init__(self, in_channels, out_channels, kernel_size, alpha_in=0.5, alpha_out=0.5, stride=1, padding=0,
                 dilation=1,groups=1, bias_attr=False, norm_layer=nn.BatchNorm2D):
        super(Conv_BN,self).__init__()
        self.conv = OctaveConv(in_channels, out_channels, kernel_size,alpha_in,alpha_out,stride,padding,
                           dilation,groups,bias_attr=bias_attr)
