...
def last_zero_init(m):
    if isinstance(m, nn.Sequential):
        m[len(m)-1].weight.set_value(paddle.zeros(m[len(m)-1].weight.shape))
    else:
...
class ContextBlock(nn.Layer):
    ...
    def reset_parameters(self):
        if self.pooling_type == 'att' and self.conv_mask_inited :
            self.conv_mask.weight=self.conv_mask.create_parameter(self.conv_mask.weight.shape, default_initializer= nn.initializer.KaimingNormal())
            self.conv_mask_inited = False
...
if __name__ == '__main__':
    network = ContextBlock(inplanes=3, ratio=16)
