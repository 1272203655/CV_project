...
# BAT   
"""
BAT(Bilinear Attentional Transforms)是2020年CVPR上的一篇论文
Non-Local Neural Networks with Grouped Bilinear Attentional Transforms
中提出的一种针对non-local网络改进的网络结构。
BAT是在传统的non-local模块上改进的可进行变形操作的新型模块
"""  
class BATransform(nn.Layer): 
    ...
    def forward(self, x):
         ...
         q = self.resize_mat(q,x.shape[3]//self.s)
...
class BATBlock(NonLocalModule):
    def __init__(self, in_channels, r=2,s=4,k=4,dropout=2,**kwargs) :
