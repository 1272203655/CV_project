...
def calc_iou(a,b):
    ...
    iw = toch.clamp(iw,min=0)
    ih = toch.clamp(ih,min= 0)
    # iw * ih = 并集面积
    ...
    IoU = intersection/ua
    return IoU
def get_target(anchors,bbox_annitation,classsificition,cuda):
    #   计算真实框和先验框的交并比
    #   anchor              num_anchors, 4
    #   bbox_annotation     num_true_boxes, 5  标注框 加上标签
    #   Iou                 num_anchors, num_true_boxes 
    IoU = calc_iou(anchors[:,:],bbox_annitation[:,:4])
    #   计算与先验框重合度最大的真实框
    IoU_max,Iou_argmax = torch.max(IoU,dim=1)
    #   寻找哪些先验框在计算loss的时候需要忽略
    targets = torch.ones_like(classsificition) * -1
    if cuda:
    ...
    #   取出每个先验框最对应的真实框
    assigned_annotations = bbox_annitation[Iou_argmax,:]
    #   将对应的种类置为1
...
def encode_bbox(assigned_annotations,positive_indices,anchor_widths,anchor_heights,anchor_ctr_x,anchor_ctr_y):
    ...
    targets = targets.t()  #转置
    return targets
class FocalLoss(nn.Module):
    def __init__(self):
        super().__init__()
    def forward(self,classifications,regressions,anchors,annotations,alpha=0.5,gamma=2.0,cuda=True):
        ...
        for j in range(batch_size):
            ...
            if len(bbox_annotation)==0:
                ...
                focal_weight = alpha_factor * torch.pow(focal_weight,gamma)  # return focal_weight^gamma
                #   计算特征点对应的交叉熵
            ...
            targets,num_positive_anchors, positive_indices, assigned_annotations = get_target(
                anchors,bbox_annotation,classification,cuda
            )
            #   首先计算交叉熵loss
            ...
            # eq 相等  where条件 where(condition,x,y) if condition x else y   正样本 alpha_factor
