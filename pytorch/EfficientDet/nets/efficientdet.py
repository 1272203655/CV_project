...
class SeparableConvBlock(nn.Module):
    def __init__(self,in_chanels,out_channels=None,norm=True,activation=False,onnx_export=False):
        super(SeparableConvBlock,self).__init__()
        if out_channels is None:
            out_channels = in_chanels
        self.depthwise_conv = Conv2dStaticSamePadding(in_chanels,in_chanels,kernel_size=3,stride=1,groups=in_chanels,bias=False)
        self.pointwise_conv = Conv2dStaticSamePadding(in_chanels,out_channels,kernel_size=1,stride=1)
        self.norm = norm
        if self.norm:
...
class BiFPN(nn.Module):
    def __init__(self,num_channels,conv_channels,first_time=False,epsilon=1e-4,onnx_export=False, attention=True):
        ...
        self.p4_downsample = MaxPool2dStaticSamePadding(3, 2)  #kernel_size=3  stride= 2
        ...
        if self.first_time:  #BiFPN属于多次堆叠 第一次堆叠时  调整通道数
            self.p5_down_channels = nn.Sequential(
                Conv2dStaticSamePadding(conv_channels[2],num_channels,kernel_size=1),
            ...
            self.p5_to_p6  = nn.Sequential(
                ...
                nn.BatchNorm2d(num_channels,momentum=0.01,eps=1e-3),
                MaxPool2dStaticSamePadding(3,2)  #下采样两倍
            )
            self.p6_to_p7 = nn.Sequential(
                MaxPool2dStaticSamePadding(3,2)
            )
            # BIFPN第一轮的时候，跳线那里并不是同一个in
            self.p4_down_channels_2 = nn.Sequential(
                Conv2dStaticSamePadding(conv_channels[1],num_channels,1),
                nn.BatchNorm2d(num_channels,momentum=0.01, eps=1e-3),
            )
            self.p5_down_channels_2 = nn.Sequential(
                Conv2dStaticSamePadding(conv_channels[2], num_channels, 1),
                nn.BatchNorm2d(num_channels, momentum=0.01, eps=1e-3),
            )
        # 简易注意力机制的weights
    ...
    def forward(self,inputs):
        ...
        """
        if self.attention:
            p3_out, p4_out,p5_out,p6_out,p7_out = self._forward_first_attention(inputs)
        else:
            p3_out, p4_out, p5_out, p6_out, p7_out = self._forward(inputs)
        return p3_out, p4_out, p5_out, p6_out, p7_out
    def _forward_first_attention(self,inputs):
        #------------------------------------------------#
        ...
        if self.first_time:
            ...
            p4_in_1 = self.p4_down_channels(p4)
            p4_in_2 = self.p4_down_channels_2(p4)
            p5_in_1 = self.p5_down_channels(p5)
            p5_in_2 = self.p5_down_channels_2(p5)
            #-------------------------------------------#
            #   对C5进行下采样，调整通道数与宽高
            #   C5 16, 16, 320 -> 8, 8, 64
            #   对P6_in进行下采样，调整宽高
            ...
            # 简单的注意力机制，用于确定更关注p7_in 还是 p6_out
    ...
    def _forward(self,inputs):
        ...
        if self.first_time:
            ...
            p3_out = self.conv3_up(self.swish(p3_in+self.p4_downsample(p4_td)))
            p4_out = self.conv4_down(self.swish(p4_in_2 + p4_td + self.p4_downsample(p3_out)))
            p5_out = self.conv5_down(
...
class BoxNet(nn.Module):
    def __init__(self,in_chanels,num_anchors,num_layers,onnx_export=False):
        super().__init__()
        self.num_layers = num_layers
        self.conv_list = nn.ModuleList(
            [SeparableConvBlock(in_chanels,in_chanels,norm=False,activation=False) for i in range(num_layers)]
        )
        # 每一个有效特征层对应的Batchnor不同
        self.bn_list = nn.ModuleList(
            [nn.ModuleList([nn.BatchNorm2d(in_chanels,momentum=0.01,eps=1e-3) for i in range(num_layers)]) for i in range(5)]
        )
        # 9
        # 4 中心，宽高
        self.header = SeparableConvBlock(in_chanels,num_anchors*4,norm=False,activation=False)
        self.swish = MemoryEfficientSwish() if not onnx_export else Swish()
    def forward(self,inputs):
        feats = []
        # 对每个特征层循环
...
class EfficientNet(nn.Module):
    def __init__(self,phi,load_weight = False):
        super().__init__()
        model = EffNet.from_pretrained(f'efficientnet-b{phi}',load_weight)
        ...
        self.model = model
    def forward(self,inputs):
        x = self.model._conv_stem(inputs)
        x = self.model._bn0(x)
        ...
        for idx,block in enumerate(self.model._blocks):
            ...
            last_x = x
        del last_x
        return feature_maps[1:]  #return P3,P4,P5  p2不要
class EfficientDetBackbone(nn.Module):
    def __init__(self,num_classes=80,phi=0,load_weight=False): # phi指的是efficientdet的版本
        super().__init__()
        self.phi = phi
        ...
        #   P7_out      4,4,64
        #------------------------------------------------------#
        self.bi_fpn = nn.Sequential(
            *[BiFPN(self.fpn_num_filters[self.phi],
            conv_channels_coef[phi],
            True if _ ==0 else False,
            attention=True if phi <6 else False) for _ in range(self.fpn_cell_repeats[phi])]
        )
        self.num_classes = num_classes
        self.regressor = BoxNet(in_chanels=self.fpn_num_filters[self.phi],
                num_anchors=num_anchors,num_layers=self.box_class_repeats[self.phi])
        self.classifier  = ClassNet(in_channels=self.fpn_num_filters[self.phi],
                num_anchors=num_anchors,num_classes=num_classes,num_layers=self.box_class_repeats[self.phi])
        self.anchors = Anchors(anchor_scale=self.anchor_scale[self.phi])
        # 获得p3,p4,p5
        self.backbone_net = EfficientNet(self.backbone_phi[self.phi],load_weight)
    def freeze_bn(self):
